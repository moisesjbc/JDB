...
void Library::saveFile( const char* filePath ) const
{
    tinyxml2::XMLDocument xmlFile;
    tinyxml2::XMLElement* libraryNode = nullptr;
    tinyxml2::XMLComment* libraryHeader = nullptr;

    // Create a header comment for the xml file.
    libraryHeader = xmlFile.NewComment( "Generated by Graphic Librarian" );

    // Create a node "library".
    libraryNode = xmlFile.NewElement( "library" );

    // Insert all previous nodes in XML file.
    xmlFile.InsertFirstChild( libraryHeader );
    xmlFile.InsertAfterChild( libraryHeader, libraryNode );
